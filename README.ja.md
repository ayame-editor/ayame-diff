...
[![ワード・構文ハイライト付きの左右差分](docs/assets/screenshot-diff.png)](docs/assets/screenshot-diff.png)
...
ブラウザは表示層であり、実行サンドボックスではありません。ネイティブの
ayame-diff プロセスが、直接パスアクセス、ファイル変更監視、ディスクを使う
外部ソート、CLI と同じ比較エンジンを提供します。

| ローカルの ayame-diff プロセス | ブラウザだけで配布する場合の境界 |
|---|---|
| 外部保存後にファイル由来の比較を監視・更新 | ページ外の任意の変更を監視する可搬な API はない |
| メモリ予算を守り、巨大 CSV/TSV 処理を `--temp-dir` へ spill | ブラウザメモリと管理ストレージの容量制限を受ける |
| CLI、ファイルマネージャー、Git カスタムツールに対応 | ブラウザ・OS固有の受け渡しに依存する |

ユーザーが許可すればブラウザ API で一部を実現できるため、WebAssembly では絶対に
不可能という主張ではありません。詳しくは
[ローカルサーバー方式を選ぶ理由](docs/design.ja.md)と
[ファイルマネージャー・Git 連携](docs/shell-integration.ja.md)を参照してください。
...
起動のたびにAPIトークンを生成し、表示されるURLに含めます。トークンのない呼び出しはAPIが
拒否し、loopbackで待ち受ける場合は`Host`ヘッダーも固定するため、閲覧中のWebサイトから
操作されることはありません。loopback 以外のアドレスは、`--allow-remote`を指定しない限り
拒否されます。リモートモードではURLを持つ人が誰でもローカルファイルを読み書きできるため、
...
- `0`: 正常終了（差分なし、またはマージ出力を完全に解決）
- `1`: `--diff-exit-code` で差分あり、または `--merge-exit-code` で未解決出力あり
- `2`: 使用法エラー（不正なフラグ・引数・両立しないオプション）
- `3`: 実行時エラー（入出力・比較・サーバー・アップデートの失敗）
- `130`: 割り込み、または明示的なキャンセル（例: `remove` の確認で中止）

明示的な exit-code flag を指定しない通常実行では、差分の有無にかかわらず
正常完了は `0` を返します。`1` は想定内の比較・マージ結果専用なので、
スクリプトは使用方法・実行時失敗（`2` と `3`）と区別できます。マージの契約は
[3-way比較](docs/three-way.ja.md)を参照してください。
