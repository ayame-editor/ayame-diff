...
(function (root, factory) {
  ...
  const CATALOG = {
    ...
    continuousView: "通し表示",
    continuousCounter: (v) => `ファイル ${v.current} / ${v.total}`,
    hunkCount: (v) => `${v.count} ハンク`,
    folderMatchScope: (v) => `${v.total} 件を比較`,
    ...
    errInternal: "内部エラーが発生しました。もう一度実行しても解決しない場合は、問題として報告してください。",
      ...
      contextToggle: "文脈", contextToggleTitle: "差分の前後に未変更行を表示",
      resultDisplay: "結果表示", resultDisplayHint: "再比較せず、差分の周囲に何を表示するかを変更します。",
      contextLines: "未変更の文脈行数", contextRegion: "未変更行", contextHidden: (v) => `${v.count} 行の未変更行を折りたたみ`,
      contextExpandUp: (v) => `上へ ${v.count} 行展開`, contextExpandDown: (v) => `下へ ${v.count} 行展開`,
      contextExpandBoth: (v) => `両端から最大 ${v.count} 行ずつ展開`, contextLoading: "未変更行を読み込み中…",
    ...
    continuousCounter: (v) => `file ${v.current} / ${v.total}`,
    hunkCount: (v) => `${v.count} ${Number(String(v.count).replace(/,/g, "")) === 1 ? "hunk" : "hunks"}`,
    folderMatchScope: (v) => `${v.total} entries compared`,
    ...
    errInternal: "Something went wrong inside ayame-diff. Report it if it happens again.",
      ...
      contextToggle: "Context", contextToggleTitle: "Show unchanged lines around differences",
      resultDisplay: "Result display", resultDisplayHint: "Changes what is shown around differences without recomputing them.",
      contextLines: "unchanged context lines", contextRegion: "Unchanged lines", contextHidden: (v) => `${v.count} unchanged lines collapsed`,
      contextExpandUp: (v) => `Expand ${v.count} lines up`, contextExpandDown: (v) => `Expand ${v.count} lines down`,
      contextExpandBoth: (v) => `Expand up to ${v.count} lines from both edges`, contextLoading: "Loading unchanged lines…",
