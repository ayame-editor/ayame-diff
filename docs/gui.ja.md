...
![ファイルパスと比較オプションをまとめたセットアップ画面](assets/screenshot-setup.png)

*よく使う設定は常に見える位置へ置き、モード固有の設定だけを必要に応じて表示します。*
...
!!! warning "ローカル・シングルユーザー専用"
    ...
    認証は追加されないため、信頼できるネットワークのアクセス制御下に置いてください。
...
| `--allow-remote` | オフ | loopback以外の受信アドレスを明示的に許可します。認証はありません。 |
